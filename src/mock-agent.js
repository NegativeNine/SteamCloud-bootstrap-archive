import { assertNoSecretFields } from './profile.js';

export class MockSteamAgent {
  #seen = new Map();
  constructor({leaseBook}) { this.leaseBook = leaseBook; }
  ...
  execute(grant, argumentsValue) {
    ...
    for (const [key, generation] of Object.entries(grant.lease_fences)) this.leaseBook.require(key, generation);
    const fingerprint = `${grant.idempotency_key}:${grant.arguments_sha256}`;
    const prior = this.#seen.get(fingerprint);
    if (prior) return {...prior, replayed:true};
    if (grant.operation === 'asf.owner.edge.fetch') throw new Error('owner edge action cannot execute in regional agent');
    const result = Object.freeze({
      schema:'campfire.action-settlement/v1', attempt_id:grant.attempt_id,
      outcome:'COMPLETED', operation:grant.operation, replayed:false,
